...
function Topic() {
	...
	this.name = "";
	this.id = 0;
	// Note that a topic profile can have many queries e.g.
	// - multiple filter queries (e.g. "Brands", "Competitors")
	// - multiple include queries
	// - multiple exclude queries
	// - multiple includeAl queries
...
function loadTopicList(xmlDoc) {
	...
	for (var i=0; i<topicFilters.length; i++) {
		...
		var queries = topicFilters[i].getElementsByTagName("query");
		if (queries.length > 0) {
			topic.query = queries[0].firstChild.nodeValue;
		}
...
function loadArticle(node, article) {
	if (node.nodeName == "source") {
		article.source = getFirstChildValue(node);
	} else if (node.nodeName == "host") {
		article.host = getFirstChildValue(node);
	} else if (node.nodeName == "article_url") {
		article.article_url = getFirstChildValue(node);
	} else if (node.nodeName == "media_provider") {
		article.media_provider = getFirstChildValue(node);
	} else if (node.nodeName == "media_type_id") {
		article.media_type_id = getFirstChildValue(node);
		...
		article.headline = getFirstChildValue(node);
	} else if (node.nodeName == "author") {
		article.author = getFirstChildValue(node);
	} else if (node.nodeName == "recipient") {
		article.recipient = getFirstChildValue(node);
	} else if (node.nodeName == "content") {
		article.content = getFirstChildValue(node);
	} else if (node.nodeName == "avatar") {
		article.avatar = getFirstChildValue(node);
...
function getFirstChildValue(node) {
...
/**
 * This object represents authentication in Radian6
 */
function Radian6Auth() {
	this.base_url = "http://sandboxapi.radian6.com/socialcloud/v1/auth/authenticate";
	this.username  = "";
	this.password = "";
	this.auth_token = "";
	this.auth_appkey = "";
	this.authenticate = doAuthenticate();
	this.status = 0;
}
...
function doAuthenticate() {
	...
	var auth_url = 
	req.onreadystatechange = doAuthenticateChange;
	req.open("GET", this.base_url, false); //synchronous
	req.setRequestHeader("auth_user", username);
	req.setRequestHeader("auth_pass", password);
	req.send();				
    if (req.readyState == 4) {
        // only if "OK"
        if (req.status == 200) {
			var xmlDoc = req.responseXML;
			doGetAuthTokens(xmlDoc);
        } else {
...
/**
 * This object represents Radain6 Timeseries data for a specific Topic and Media type
 */
function TimeSeriesData() {
	...
	this.media_id = 0;
	this.data = [];
	this.get = getTimesSeriesData(auth);
}